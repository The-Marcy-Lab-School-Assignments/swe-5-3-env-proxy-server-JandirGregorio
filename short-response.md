...
**Your answer here**: Sites like Giphy require an API Key to make requests to their servers. If we expose the API Key in the frontent, users can access it through the broswer developer tools in the _Sources_ tab or _Network_ tab. It's unsafe because there might costs associated with the API key or quota and the user could exploit by impersonating us through making exesive requests. This could be costly and get us banned from the third party APIs.
...
**Your answer here**: An environement variable is a hidden variable that lives in the host's machine (local computer o server-host like Render). We store sensitive data like API keys in a .env file because the `key="value"` data is accessible in Node through the `process.env` object. We must list the .env file in the .gitignore file so that .env gets ignored when commit and push our code to GitHub. If this is not done, the API key can be pushed and will live in our commit history, making accessible to anyone on the internet.
